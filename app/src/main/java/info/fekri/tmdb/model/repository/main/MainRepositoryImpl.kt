...
class MainRepositoryImpl(
    ...
    override suspend fun getActionMovies(isInternetConnected: Boolean): List<QueryResult> {
        if (isInternetConnected) return apiService.getMovieByQuery(query = "Action").results

        return listOf()
    }

    override suspend fun getAnimationMovies(isInternetConnected: Boolean): List<QueryResult> {
        if (isInternetConnected) return apiService.getMovieByQuery(query = "Animation").results

        return listOf()
    }

    override suspend fun getMysteryMovies(isInternetConnected: Boolean): List<QueryResult> {
        if (isInternetConnected) return apiService.getMovieByQuery(query = "Mystery").results
